...
import com.example.servicea.config.FtpConfig;
...
public class FtpClientService {
     ...
     * 上传文件到远程FTP服务器
     */
    public String uploadRequest(String content) throws IOException {
        String requestId = UUID.randomUUID().toString();
        String fileName = "request_" + requestId + ".json";
        ...
        try {
            // 确保目录存在
            ftpClient.makeDirectory(ftpConfig.getRequestDirectory());
            ...
            if (uploaded) {
                log.info("Request uploaded successfully: {}", fileName);
                return requestId;
            } else {
                throw new IOException("Failed to upload request file: " + fileName);
            }
        } finally {
     ...
     * 从本地FTP服务器下载响应文件
     */
    public String downloadResponse(String requestId, long timeoutMs) throws IOException {
            ...
            try {
                ...
                if (files.length > 0) {
                    ...
                    if (downloaded) {
                        ...
                        log.info("Response downloaded successfully: {}", fileName);
        ...
        throw new IOException("Timeout waiting for response: " + fileName);
    }
